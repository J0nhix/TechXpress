...
function Pago( props ){
    ...
    const { data, setData, post, processing, errors, reset } = useForm({
        productos: props.data.productos,
        precio: props.data.precio,
        puntos: props.data.puntos,
    ...
    const validateCardNumber = (value) => {
      ...
      const submit = (e) => {
        ...
        if (!validateCardNumber(data.cardNumber)) {
          alert('No has puesto bien el número de tarjeta. Por favor, verifica que hay 16 digitos en el número de tarjeta.');
          return;
        }
    
        // Validar expiryDate
        if (!validateExpiryDate(data.expiryDate)) {
          alert('No has puesto bien la fecha de expiración. Por favor, verifica que has puesto la fecha de esta forma MM/YY.');
          return;
        }
    
        // Validar CVV
        if (!validateCVV(data.cvv)) {
          alert('No has puesto bien el numero de CVV. Por favor, verifica que hay 3 digitos en el número de CVV.');
          return;
        }
    
        // Si todas las validaciones pasan, realizar la acción deseada (por ejemplo, enviar el formulario)
        post(route('pedido.procesopago'), {
          onSuccess: () => {
